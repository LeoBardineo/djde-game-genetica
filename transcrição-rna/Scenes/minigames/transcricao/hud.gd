...
@onready var stars_label: RichTextLabel = $GameOverPanel/VBoxContainer/StarsLabel
...
func _animate_stars(earned_stars: int) -> void:
	...
	for i in range(earned_stars):
		...
		if star_full_texture:
			...
			star_rect.modulate = Color(1.0, 0.85, 0.1) 
		
		var tween = create_tween()
		star_rect.scale = Vector2(0.3, 0.3)
		tween.tween_property(star_rect, "scale", Vector2(1.2, 1.2), 0.15).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		tween.tween_property(star_rect, "scale", Vector2.ONE, 0.1)
